...
async function loadProfile(){
 ...
 const ent=await rest("organisatie_module?select=actief&organisatie_id=eq."+encodeURIComponent(u.organisatie_id)+"&module_sleutel=eq.werkbonnen&actief=eq.true");if(!ent?.length)throw new Error("Werkbonnen is voor jouw organisatie nog niet geactiveerd.");
 const rr=await rest("werkbon_gebruiker?select=rol&gebruiker_id=eq."+encodeURIComponent(u.id)+"&organisatie_id=eq."+encodeURIComponent(u.organisatie_id));wbRole=rr?.[0]?.rol;if(!wbRole)throw new Error("Je account heeft nog geen Werkbonnen-rol.");
 profile=u;$("userBadge").textContent=u.naam||u.email||"";$("orgLabel").textContent=u.organisatie.naam||"";$("roleLabel").textContent=ROLE_LABELS[wbRole]||wbRole;$("newWorkorderButton").hidden=!office();$("manageButton").hidden=!admin();showPortal();try{await refs();await loadInbox();await list()}catch(e){err($("accessMessage"),"Inloggen gelukt, maar gegevens laden mislukt: "+(e.message||"onbekende fout"));console.error("Werkbonnen initialisatie:",e)}return true
}
async function refs(){
...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 const refresh=async()=>{try{await refs();await loadInbox();await list()}catch(e){err($("accessMessage"),e.message||"Verversen mislukt.")}};$("refreshHeaderButton").onclick=refresh;$("refreshContentButton").onclick=refresh;
 ...
 document.querySelectorAll(".side-nav-item[data-scroll]").forEach(b=>b.onclick=()=>{const target=$(b.dataset.scroll==="workorders"?"workordersPanel":b.dataset.scroll);if(target)target.scrollIntoView({behavior:"smooth",block:"start"});document.querySelectorAll(".side-nav-item").forEach(x=>x.classList.remove("active"));b.classList.add("active")});
 $("sideManageButton").onclick=()=>$("manageButton").click();
 const syncManage=()=>{$("sideManageButton").hidden=$("manageButton").hidden};new MutationObserver(syncManage).observe($("manageButton"),{attributes:true});syncManage();
